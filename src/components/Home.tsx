export function Home() {
  return (
    <div className="flex flex-col items-center justify-center pt-20 pb-6 text-center text-gray-800 dark:text-gray-100">
      <h1 className="text-4xl font-bold md:text-6xl">
        Hi, I'm Gonçalo Guimarães
      </h1>
      <p className="mt-4 text-xl font-semibold text-bold">
        Full Stack Web Developer
      </p>
      <p className="mt-4 text-lg">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Nobis,
        laudantium? Consectetur laboriosam, dicta molestiae unde maiores
        deserunt. Quae laborum pariatur sapiente, ut exercitationem aliquam
        nostrum ipsum consectetur ea a laudantium!
      </p>
    </div>
  );
}
